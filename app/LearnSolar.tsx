...
export default function LearnRiddles() {
    ...
    const [levelInSolar, setLevelInSolar] = useState(0);
    ...
    const fetchUserInfo = useCallback(async () => {
        ...
        if (uniqueId) {
            ...
            if (user) {
                setUserInfo(user);
                setLevelInSolar(user.levelInSolar);
            } else {
    ...
    const renderItem: ListRenderItem<number | string> = ({ item, index }) => {
        ...
        if (!isBonus) {
            isEnabled = item <= levelInSolar;
        } else {
            const prevRegularLevel = levels[index + 1];
            isEnabled = typeof prevRegularLevel === 'number' && prevRegularLevel <= levelInSolar;
        }
        ...
        return (
            <TouchableOpacity
                ...
                onPress={() => isEnabled && !isBonus ? navigation.navigate('SolarQuizLevels', { level: item }) : navigation.navigate('BonusSolarQuiz', { level: item })}
                disabled={!isEnabled}
            >
