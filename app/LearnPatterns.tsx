...
export default function LearnRiddles() {
    ...
    const [levelInPattern, setLevelInPatterns] = useState(0);
    ...
    const fetchUserInfo = useCallback(async () => {
        ...
        if (uniqueId) {
            ...
            if (user) {
                setUserInfo(user);
                setLevelInPatterns(user.levelInPattern);
            } else {
    ...
    const renderItem: ListRenderItem<number | string> = ({ item, index }) => {
        ...
        if (!isBonus) {
            isEnabled = item <= levelInPattern;
        } else {
            const prevRegularLevel = levels[index + 1];
            isEnabled = typeof prevRegularLevel === 'number' && prevRegularLevel <= levelInPattern;
        }
        ...
        return (
            <TouchableOpacity
                ...
                onPress={() => isEnabled && !isBonus ? navigation.navigate('PatternQuizLevels', { level: item }) : navigation.navigate('BonusPatternQuiz', { level: item })}
                disabled={!isEnabled}
            >
